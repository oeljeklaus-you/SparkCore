...
private[spark] class ExecutorRunner(
  ...
  //TODO start()方法先创建线程，然后通过线程调用fetchAndRunExecutor()方法来启动Java子进程
  def start() {
    workerThread = new Thread("ExecutorRunner for " + fullId) {
      override def run() { fetchAndRunExecutor() }
    }
    //TODO 启动线程也就是变相的启动fetchAndRunExecutor()方法
    workerThread.start()
   ...
   */
  //TODO 线程对象会调用该方法启动子进程
  def fetchAndRunExecutor() {
    try {
      // Launch the process
      //TODO 创建用于Executor运行的builder
      ...
      //TODO 真正启动一个子进程主要是CoarseGrainedExecutorBackend
      ...
      //TODO 向Worker通信，发送Executor状态已经改变
